...
1. **Verb data (`VERBS`, `src/data/verbs.js`)** — the source-of-truth data model. Each verb has:
   ...
   - `sentences`/`pronounSentences`/`negativeSentences`: example-sentence data powering the "complete the sentence" question styles; each variant's `validFor` tag (schema in `docs/SENTENCE_FRAMES.md`) lists sibling verb ids whose same-person form would also correctly complete that exact sentence, governing cross-verb distractor eligibility
