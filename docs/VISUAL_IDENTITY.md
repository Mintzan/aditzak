...
**Status: fully implemented.** This started as a from-scratch redesign specification; implementation began 2026-07-02 and landed in small, independently-shippable PRs (iterations 1–10) rather than one big rewrite. Track A (§2–5's palette/typography/component geometry) and Track B (round 6's full 8-placement mascot rollout) are both complete — every row in the "Gap analysis" table below and the "Mascot placement plan" table is shipped. Iteration 10's trigger for the last placement (the in-lesson Haserre callout) was a user-delegated judgment call ("follow your intuition") rather than a guide-specified rule — see its `docs/DECISIONS.md` entry for the reasoning, in case it needs revisiting.
...
| Mascot / character | none | full mascot system, **decided (round 6) to be a central part of the app** — see "Mascot placement plan" below, not just the feedback-drawer micro-moments §1C originally scoped | **Shipped** — all 8 placements (iterations 6–10) |
...
| In-lesson error-prone-pattern callout | `ExerciseScreen.jsx` (question prompt area) | Haserre | Already scoped in §1C. | **Shipped (iteration 10)** — per-learner, 2+ prior misses on the exact form (`errorStats`), not a static per-verb flag |
