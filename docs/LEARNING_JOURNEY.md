...
|---|---|---|---|---|---|
| 34 | **Ahalera — Permissions & Capability** — `dezaket`/`naiteke` contrasted with periphrastic `ahal izan`, plus `ukan`'s NOR-NORK object axis (`zaitzaket`-type forms, #352) across all three Ahalera sub-tenses (present/hypothetical/past), plus `esan`/`eman`'s ditransitive Ahalera (`diezaioket`-type forms, #366) | "I can come." / "I could (have) ..." | NOR/NOR-NORK [P]; dative [R] | §2, §3, VERB_COVERAGE §5 (`ahal`/`ezin`) | available (#148, object axis added by #352, esan/eman ditransitive [R] added by #366) — `izan`/`ukan` potential, form-only; remaining dative [R] paradigms deferred |
